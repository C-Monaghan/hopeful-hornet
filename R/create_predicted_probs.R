  ...
  predicted_probs <- imap(augmented_data, function(by_sub_blocks, parent) {
    imap(by_sub_blocks, function(by_sizes, sub_block) {
      imap(by_sizes, function(data_list, size_label) {
        map2(data_list, model_fits[[parent]][[sub_block]][[size_label]], function(data_augment, model) {
          ...
          if (is.null(data_augment) || is.null(model)) return(NULL)
          ...
          tryCatch({
            ...
            split_rows <- split(seq_len(nrow(probs)), ceiling(seq_along(seq_len(nrow(probs))) / 3))
